def header_descriere() -> str:
    return """
    <a href="/kosovo/capitala">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Capitala</button>
    </a>
    <a href="/kosovo/steag">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Steag</button>
    </a><br><br>
    <h1>Aceasta este descrierea pentru kosovo</h1><br><br>
    """

def header_capitala() -> str:
    return """
    <a href="/kosovo">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Descriere</button>
    </a>
    <a href="/kosovo/steag">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Steag</button>
    </a><br><br>
    <h1>Aceasta este capitala pentru kosovo</h1><br><br>
    """

def header_steag() -> str:
    return """
    <a href="/kosovo">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Descriere</button>
    </a>
    <a href="/kosovo/capitala">
        <button style="padding: 10px 20px; font-size: 16px;">Vezi Capitala</button>
    </a><br><br>
    <h1>Acesta este steagul pentru kosovo</h1><br><br>
    """
